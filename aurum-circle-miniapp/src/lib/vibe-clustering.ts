...
export class VibeClusterer {
  ...
  /**
   * Generate a complete vibe profile (backward compatibility)
   */
  async generateVibeProfile(
    embedding: number[],
    face: ProcessedFace
  ): Promise<VibeProfile> {
    const vibeTags = await this.generateVibeTags(embedding, face);

    // Extract primary and secondary vibes
    const primary = vibeTags[0] || "Authentic";
    const secondary = vibeTags[1] || "Natural";

    // Map vibe tags to traits
    const traits = vibeTags.slice(2); // Remaining tags as traits

    // Determine aesthetic and energy based on primary vibe
    const aesthetic = this.getAestheticFromVibe(primary);
    const energy = this.getEnergyFromVibe(primary);
  ...
  /**
   * Get aesthetic category from vibe
   */
  private getAestheticFromVibe(vibe: string): string {
    ...
    return aestheticMap[vibe] || "natural";
  }

  /**
   * Get energy category from vibe
   */
  private getEnergyFromVibe(vibe: string): string {
    ...
    return energyMap[vibe] || "balanced";
  }
}
