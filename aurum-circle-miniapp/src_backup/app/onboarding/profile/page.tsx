...
import BirthDataForm from "@/components/bazi/birth-data-form";
...
export default function ProfileSetupPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    bio: "",
    birthData: undefined as BirthData | undefined,
  });
    ...
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-text-muted text-sm">
          Your unique mystic tags based on your BaZi birth chart
        </p>
      </div>
      ...
      {formData.birthData && (
        <Card className="p-4 bg-card-muted rounded-lg">
          ...
          <div className="space-y-2 text-sm">
            <div>
              <strong>Date:</strong>{" "}
              {formData.birthData.date.toLocaleDateString()}
            </div>
            <div>
              <strong>Time:</strong>{" "}
              {formData.birthData.date.toLocaleTimeString()}
            </div>
            <div>
              <strong>Timezone:</strong> {formData.birthData.timezone}
            </div>
            <div>
              <strong>Privacy:</strong>{" "}
              {formData.birthData.isPrivate ? "Private" : "Public"}
            </div>
          </div>
        </Card>
      )}
      ...
      {/* Mystic Tags Preview */}
      <Card className="p-4 bg-card-muted rounded-lg">
        <h3 className="font-medium text-text-primary mb-3">Your Mystic Tags</h3>
        <div className="space-y-3">
          <div className="text-center py-8">
            <div className="text-4xl mb-2">🔮</div>
            <p className="text-text-muted">
              Your BaZi mystic tags will be displayed here after profile
              creation
            </p>
          </div>
        </div>
      ...
      <Card className="p-4 bg-card-muted rounded-lg">
        ...
        <div className="space-y-2 text-sm">
          ...
          {formData.birthData && (
            <div>
              <strong>BaZi:</strong> Birth data provided for mystic tag
              generation
            </div>
          )}
        </div>
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <Card className="w-full max-w-md p-8">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-text-muted">Step {step} of 4</p>
        </div>
        ...
        {step === 3 && (
          <div className="space-y-6">
            <div className="text-center">
              ...
              <p className="text-text-muted text-sm">
                Provide your birth data for mystic tag generation
              </p>
            </div>
            <BirthDataForm
              initialData={formData.birthData}
              onSubmit={(birthData) => {
                handleBirthDataChange(birthData);
                setStep(4);
              }}
              onSkip={() => setStep(4)}
            />
