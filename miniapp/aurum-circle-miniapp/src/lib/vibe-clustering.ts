...
export class VibeClusterer {
  ...
  private readonly SIGNATURE_VIBES = [
    { name: 'Mystic', traits: ['mysterious', 'sophisticated', 'cool'], rarity: 0.05 },
    ...
    { name: 'Classic', traits: ['elegant', 'sophisticated', 'timeless'], rarity: 0.09 },
    { name: 'Artistic', traits: ['creative', 'unique', 'authentic'], rarity: 0.08 },
    { name: 'Luminous', traits: ['radiant', 'bright', 'energetic'], rarity: 0.06 },
    { name: 'Refined', traits: ['polished', 'cultured', 'classic'], rarity: 0.07 },
    { name: 'Magnetic', traits: ['powerful', 'commanding', 'intense'], rarity: 0.05 },
    { name: 'Gentle', traits: ['kind', 'tender', 'soft'], rarity: 0.10 },
    { name: 'Enigmatic', traits: ['intriguing', 'deep', 'mysterious'], rarity: 0.06 },
    { name: 'Spirited', traits: ['fun', 'charming', 'playful'], rarity: 0.09 },
    { name: 'Serene', traits: ['peaceful', 'balanced', 'calm'], rarity: 0.08 },
    { name: 'Dynamic', traits: ['energetic', 'lively', 'vibrant'], rarity: 0.07 }
  ]
  
  /**
   * Generate vibe tags for a face embedding
   */
  async generateVibeTags(embedding: number[], face: ProcessedFace): Promise<string[]> {
    try {
      // Reduce embedding to vibe dimensions using simulated PCA
      const vibeVector = this.extractVibeFeatures(embedding, face)
      
      // Calculate distances to vibe clusters
      const vibeScores = this.calculateVibeAffinities(vibeVector, face)
      
      // Select primary and secondary vibes
      const selectedVibes = this.selectVibes(vibeScores, face)
      
      // Add signature vibe if applicable
      const signatureVibe = this.detectSignatureVibe(vibeScores, face)
      if (signatureVibe) {
   ...
   */
  private extractVibeFeatures(embedding: number[], face: ProcessedFace): number[] {
   ...
   */
  private extractIntensityFeature(embedding: number[], face: ProcessedFace): number {
    // Use embedding dimensions that correlate with eye/eyebrow intensity
    const eyeRegion = embedding.slice(0, 100)
    const intensity = eyeRegion.reduce((sum, val, idx) => {
   ...
   */
  private extractWarmthFeature(embedding: number[], face: ProcessedFace): number {
   ...
   */
  private extractSoftnessFeature(embedding: number[], face: ProcessedFace): number {
    // Use face shape features for softness
    const faceShape = embedding.slice(300, 400)
    const softness = faceShape.reduce((sum, val, idx) => {
   ...
   */
  private extractStyleFeature(embedding: number[], face: ProcessedFace): number {
    // Use general features for style detection
    const generalFeatures = embedding.slice(400, 512)
    const modernity = generalFeatures.reduce((sum, val, idx) => {
   ...
   */
  private extractEnergyFeature(embedding: number[], face: ProcessedFace): number {
    // Combine multiple regions for energy detection
    const eyeEnergy = embedding.slice(0, 100).reduce((sum, val) => sum + Math.abs(val), 0) / 100
    const mouthEnergy = embedding.slice(200, 300).reduce((sum, val) => sum + Math.abs(val), 0) / 100
   ...
   */
  private extractSophisticationFeature(embedding: number[], face: ProcessedFace): number {
    // Use nose and overall face features for sophistication
    const noseRegion = embedding.slice(100, 200)
    const sophistication = noseRegion.reduce((sum, val, idx) => {
   ...
   */
  private extractNaturalnessFeature(embedding: number[], face: ProcessedFace): number {
    // Look for balanced, non-extreme features
    const balance = embedding.reduce((sum, val) => sum + Math.abs(val), 0) / embedding.length
   ...
   */
  private extractUniquenessFeature(embedding: number[], face: ProcessedFace): number {
    // Look for distinctive feature combinations
    const variance = embedding.reduce((sum, val, idx, arr) => {
   ...
   */
  private calculateVibeAffinities(vibeVector: number[], face: ProcessedFace): Map<string, number> {
   ...
   */
  private selectVibes(affinities: Map<string, number>, face: ProcessedFace): string[] {
    // Sort vibes by affinity
    const sortedVibes = Array.from(affinities.entries())
      .sort(([, a], [, b]) => b - a)
   ...
   */
  private detectSignatureVibe(affinities: Map<string, number>, face: ProcessedFace): string | null {
    for (const signature of this.SIGNATURE_VIBES) {
      ...
      if (avgMatch > 0.5 && coverageRatio >= 0.6) {
        ...
        if (Math.random() < signature.rarity * 10) { // Boost probability for demo
