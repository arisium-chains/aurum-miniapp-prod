...
CREATE TABLE IF NOT EXISTS patches (
    ...
    security_analysis TEXT, -- JSON string with security findings
    performance_impact TEXT, -- JSON string with performance analysis
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS build_history (
    ...
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    duration_ms INTEGER,
    triggered_by TEXT -- 'manual', 'auto', 'webhook'
);
...
CREATE TABLE IF NOT EXISTS security_scans (
    ...
    findings TEXT NOT NULL, -- JSON string with security findings
    severity TEXT NOT NULL CHECK (severity IN ('low', 'medium', 'high', 'critical')),
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (patch_id) REFERENCES patches(id) ON DELETE CASCADE
);
