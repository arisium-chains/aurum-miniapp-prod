...
function detectAIGeneration(imageBase64: string, rawScores: RawScoreResult): {
  is_ai_generated: boolean
  confidence: number
  indicators: string[]
  detection_method: string
  ...
  if (rawScores.golden_ratio_analysis) {
    ...
    const variance = measurements.reduce((acc, val, _, arr) => {
...
function detectFilters(imageBase64: string, rawScores: RawScoreResult): {
  filter_level: 'none' | 'light' | 'moderate' | 'heavy'
  confidence: number
  detected_filters: string[]
  authenticity_impact: number
...
// Texture Analysis
function analyzeTexture(imageBase64: string, rawScores: RawScoreResult): {
  skin_smoothing: number
  artificial_enhancement: number
  texture_consistency: number
} {
  ...
  if (rawScores.style_rating > rawScores.authenticity + 20) {
    artificialEnhancement = (rawScores.style_rating - rawScores.authenticity) * 0.8
  }
  ...
  return {
    skin_smoothing: Math.min(100, Math.max(0, Math.round(skinSmoothing * 10) / 10)),
    artificial_enhancement: Math.min(100, Math.max(0, Math.round(artificialEnhancement * 10) / 10)),
    texture_consistency: Math.min(100, Math.max(0, Math.round(textureConsistency * 10) / 10))
  }
}
...
function calculateHumanPerceptionAdjustments(
  ...
  if (geometricAnalysis.distortion_detected && geometricAnalysis.severity > 0.3) {
  ...
  if (rawScores.composite_score > 90 && 
    ...
    const protectionFactor = Math.min(1.1, adjustedScore / rawScores.composite_score + 0.1)
...
async function interpretFacialScore(imageBase64: string): Promise<InterpreterResult> {
  ...
  const shouldAdjust = Math.abs(humanAdjustments.adjusted_composite - humanAdjustments.raw_composite) > 2
  const adjustmentFactor = humanAdjustments.adjusted_composite / humanAdjustments.raw_composite
  
  const reasoning: string[] = []
  if (aiDetection.is_ai_generated) reasoning.push('AI generation detected')
  if (filterDetection.filter_level !== 'none') reasoning.push(`${filterDetection.filter_level} filtering`)
  if (geometricAnalysis.distortion_detected) reasoning.push('Geometric distortion')
  if (textureAnalysis.skin_smoothing > 40) reasoning.push('Texture manipulation')
  
  // Apply adjustments to raw scores
  const finalResult: InterpreterResult = {
    ...rawScores,
    composite_score: humanAdjustments.adjusted_composite,
    authenticity: Math.max(5, rawScores.authenticity - filterDetection.authenticity_impact),
    confidence: Math.max(0.5, Math.min(0.98, rawScores.confidence + humanAdjustments.confidence_modifier)),
    interpreter_analysis: {
      ...
      final_verdict: {
        ...
        reasoning: reasoning.length > 0 ? reasoning : ['No significant issues detected']
      }
    }
  }
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (image.length > 2000000) { // ~2MB limit for base64
      return NextResponse.json({
        success: false,
    ...
    return NextResponse.json({
      ...
      debug: {
        ...
        processing_pipeline: 'raw_scoring -> ai_detection -> filter_analysis -> geometric_check -> texture_analysis -> human_adjustment'
      }
    })
