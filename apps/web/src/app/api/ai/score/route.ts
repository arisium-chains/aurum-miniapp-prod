...
function analyzeImageCharacteristics(imageBase64: string) {
  ...
  const hasComplexPatterns =
    (imageBase64.match(/[A-Z]/g) || []).length > imageBase64.length * 0.3;

  return {
    estimatedQuality: hasHighEntropy
      ? 'high'
      : hasLowEntropy
        ? 'low'
        : 'medium',
...
function checkFacialPlausibility(goldenRatioAnalysis: any): {
  valid: boolean;
  reason?: string;
} {
  ...
  for (const [measurement, value] of Object.entries(measurements)) {
    const threshold =
      EXTREME_THRESHOLDS[measurement as keyof typeof EXTREME_THRESHOLDS];
    const numValue = value as number;
    if (threshold && (numValue < threshold.min || numValue > threshold.max)) {
      return {
        valid: false,
        reason: `Facial structure outside scoring bounds: ${measurement} deviation too extreme (${numValue.toFixed(1)}%)`,
      };
    }
  }

  // Check for multiple poor measurements (extremely rare cases only)
  const poorMeasurements = Object.values(measurements).filter(
    (value: any) => value < 10
  ).length;
  if (poorMeasurements >= 8) {
    // Almost impossible to trigger
    return {
      valid: false,
      reason:
        'Multiple facial proportions severely distorted - likely corrupted image data',
    };
  }

  // Essentially disable perfect measurement rejection for high-quality photos
  const perfectMeasurements = Object.values(measurements).filter(
    (value: any) => value >= 99
  ).length;
  if (perfectMeasurements >= 11) {
    // All measurements must be exactly 99%+ (impossible with our scoring)
    return {
      valid: false,
      reason:
        'Facial proportions unrealistically perfect - likely synthetic image',
    };
  }
...
function simulateGoldenRatioAnalysis(imageBase64: string) {
  ...
  const averageAdherence =
    ratioScores.reduce((sum, score) => sum + score, 0) / ratioScores.length;
...
function generateProportionScore(
  minRange: number,
  maxRange: number,
  idealRatio: number
): number {
...
function getBestRatio(measurements: any): string {
  ...
  return (
    ratioDescriptions[ratioNames[bestRatioIndex]] || 'Overall facial harmony'
  );
}
...
function getSuggestedImprovements(measurements: any): string[] {
  ...
  if (measurements.faceRatio < 60) {
    suggestions.push(
      'Face proportions might improve with portrait orientation'
    );
  }
  if (measurements.lipNoseRatio < 65) {
    suggestions.push(
      'Lower face proportions could be enhanced with better lighting'
    );
  }

  return suggestions.length > 0
    ? suggestions
    : ['Excellent golden ratio adherence across all measurements'];
}
...
function generateMockScore(
  ...
  const plausibilityCheck = checkFacialPlausibility(
    analysis.goldenRatioAnalysis
  );
  if (!plausibilityCheck.valid) {
    console.log(
      'Facial plausibility rejection:',
      plausibilityCheck.reason,
      'Measurements:',
      analysis.goldenRatioAnalysis.measurements
    );
    throw new Error(
      plausibilityCheck.reason || 'Facial structure outside scoring bounds'
    );
  }
  ...
  const attractiveness = generateAttractivenessScore(
    analysis,
    scoreProfile.attractiveness
  );
  const photo_quality = generateQualityScore(analysis, scoreProfile.quality);
  const authenticity = generateAuthenticityScore(
    analysis,
    scoreProfile.authenticity
  );
  const style_rating = generateCorrelatedScore(
    scoreProfile.style,
    15,
    80,
    attractiveness * 0.3
  ...
  let composite_score =
    attractiveness * 0.5 + // Increased from 35% to 50%
    authenticity * 0.3 + // Increased from 25% to 30%
  ...
  const confidence = Math.max(0.75, Math.min(0.98, 0.9 - scoreVariance / 120)); // Higher base, lower variance impact

  const processing_time =
    1.2 + Math.random() * 2.5 + analysis.fileSize / 100000; // Larger files take longer
  ...
  return {
    ...
    golden_ratio_analysis: {
      overall_adherence:
        Math.round(analysis.goldenRatioAnalysis.overallAdherence * 10) / 10,
      dominant_ratio: analysis.goldenRatioAnalysis.dominantRatio,
      measurements: {
        facial_thirds:
          Math.round(
            analysis.goldenRatioAnalysis.measurements.facialThirds * 10
          ) / 10,
        face_ratio:
          Math.round(analysis.goldenRatioAnalysis.measurements.faceRatio * 10) /
          10,
        eye_spacing:
          Math.round(
            analysis.goldenRatioAnalysis.measurements.eyeSpacing * 10
          ) / 10,
        nose_ratio:
          Math.round(analysis.goldenRatioAnalysis.measurements.noseRatio * 10) /
          10,
        lip_nose_ratio:
          Math.round(
            analysis.goldenRatioAnalysis.measurements.lipNoseRatio * 10
          ) / 10,
        eye_brow_ratio:
          Math.round(
            analysis.goldenRatioAnalysis.measurements.eyeBrowRatio * 10
          ) / 10,
        lower_face_ratio:
          Math.round(
            analysis.goldenRatioAnalysis.measurements.lowerFaceRatio * 10
          ) / 10,
        jaw_temple_ratio:
          Math.round(
            analysis.goldenRatioAnalysis.measurements.jawToTempleRatio * 10
          ) / 10,
        iris_eye_ratio:
          Math.round(
            analysis.goldenRatioAnalysis.measurements.irisToEyeRatio * 10
          ) / 10,
        vertical_symmetry:
          Math.round(
            analysis.goldenRatioAnalysis.measurements.verticalSymmetry * 10
          ) / 10,
        cheekbone_ratio:
          Math.round(
            analysis.goldenRatioAnalysis.measurements.cheekboneRatio * 10
          ) / 10,
      },
      improvements: analysis.goldenRatioAnalysis.improvement,
      confidence:
        Math.round(analysis.goldenRatioAnalysis.confidence * 100) / 100,
    },
  };
}
...
function getScoreProfile(analysis: any, scenario?: string) {
  ...
  const qualityMultiplier =
    analysis.estimatedQuality === 'high'
      ? 1.4
      : analysis.estimatedQuality === 'low'
        ? 0.5
        : 0.8; // More harsh for low quality
...
function generateAttractivenessScore(
  analysis: any,
  profile: { base: number; variance: number }
): number {
  ...
  const qualityMultiplier =
    analysis.estimatedQuality === 'high'
      ? 1.25 // Higher bonus for high quality
      : analysis.estimatedQuality === 'medium'
        ? 0.85 // Penalty for medium
...
function generateCorrelatedScore(
  profile: { base: number; variance: number },
  min: number = 5,
  max: number = 95,
...
function generateAuthenticityScore(
  analysis: any,
  profile: { base: number; variance: number }
): number {
      ...
      return NextResponse.json(
        {
          success: false,
          message:
            'No face detected in image. Please upload a clearer portrait photo.',
        },
        { status: 400 }
      );
    }
      ...
      return NextResponse.json(
        {
          success: false,
          message:
            'Image quality too low for reliable scoring. Please try a clearer photo.',
        },
        { status: 400 }
      );
    }
