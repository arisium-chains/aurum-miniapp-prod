...
export class RealFaceDetector {
  ...
  async detectFaces(
    ...
    try {
      ...
      // Process predictions
      const faces = await this.processPredictions(
        predictions,
        imageTensor.shape
      );
  ...
  private preprocessImage(
    ...
    const resized = tf.image.resizeBilinear(tensor as tf.Tensor3D, [192, 192]);
  ...
  async extractFaceCrop(
    ...
    try {
      ...
      const cropWidth = Math.min(
        imageTensor.shape[1] - cropX,
        width * (1 + 2 * padding)
      );
      const cropHeight = Math.min(
        imageTensor.shape[0] - cropY,
        height * (1 + 2 * padding)
      );
      ...
      const cropped = tf.image.cropAndResize(
        ...
        [
          [
            cropY / imageTensor.shape[0],
            cropX / imageTensor.shape[1],
            (cropY + cropHeight) / imageTensor.shape[0],
            (cropX + cropWidth) / imageTensor.shape[1],
          ],
        ],
        [0],
