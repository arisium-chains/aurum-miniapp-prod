...
export class RealFaceEmbeddingExtractor {
  ...
  async extractEmbedding(
    ...
    try {
      ...
      const faceCrop = await this.preprocessFaceForEmbedding(
        imageData,
        faceDetection
      );
      ...
      const quality = this.calculateEmbeddingQuality(
        faceDetection,
        embedding,
        imageElement
      );
  ...
  private async preprocessFaceForEmbedding(
    ...
    const faceCrop = await realFaceDetector.extractFaceCrop(
      imageData,
      face,
      112
    );
  ...
  private calculateEmbeddingQuality(
    ...
    const faceQuality = realFaceDetector.calculateFaceQuality(
      face,
      imageDims.width,
      imageDims.height
    );
    ...
    const overallQuality =
      faceQuality.quality * 0.5 + magnitudeScore * 0.25 + diversityScore * 0.25;
